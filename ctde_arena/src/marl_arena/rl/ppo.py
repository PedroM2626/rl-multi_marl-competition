...
class PPOTrainer:
    ...
    def update_ctde_vd(
        self,
        actor: ActorNetwork,
        critic: ValueDecompositionCriticNetwork,
        buffer: RolloutBuffer,
        last_value: float,
        gamma: float,
        gae_lambda: float,
    ...
    def update_ctde_comm(
        self,
        actor: CommActorNetwork,
        critic: CentralizedCriticNetwork,
        buffer: RolloutBuffer,
        last_value: float,
        gamma: float,
        ...
        for _ in range(self.ppo_epochs):
            ...
            for start in range(0, sample_count, self.batch_size):
                ...
                log_probs, entropy = actor.evaluate(team_obs_batch, slot_batch, actions_batch)
                values = critic(global_batch)
